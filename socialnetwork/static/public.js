$("#letter").on("click", ".button", function () {
    ...
    $.ajax({
        ...
        success: function (json) {
            var inputvalue = formparent.find("input[name='comment']").val();
            var a = contantdata;
            formparent.find("input[name='comment']").val('');
            //var value = document.getElementsByClassName('commentc_content')[0].value;
            //document.getElementsByClassName('commentc_content')[0].value = '';
            var items = json[0];
